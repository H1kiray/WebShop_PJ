...
// Перевірка на унікальність назви секції
if (isset($_POST['add_section'])) {
    $stmt = $db->prepare("SELECT COUNT(*) FROM section WHERE name = ?");
    ...
    if (!$sectionExists) {
        ...
        $errorMsg = 'Секція з такою назвою вже існує.';
    }
}
...
if (isset($_POST['add_product'])) {
    ...
    if (!$productExists) {
        // Вставка нового товару, якщо назва унікальна
        ...
        // Використання PRG для запобігання повторному відправленню форми
        header('Location: edit_products.php');
        exit();
    } else {
        // Виведення повідомлення про помилку, якщо назва товару не унікальна
        $errorMsg = 'Товар з такою назвою вже існує.';
    }
}
...
<body>
    ...
    <?php if ($sectionAdded): ?>
        <p class="success-message">Нову секцію успішно додано!</p>
    <?php elseif ($productAdded): ?>
        <p class="success-message">Новий товар успішно додано!</p>
    <?php endif; ?>
    ...
    <div class="add-section-container">
        ...
        <form method="post">
            <div class="form-group">
                ...
                <?php if ($_SERVER['REQUEST_METHOD'] == 'POST' && $sectionExists): ?>
                    <div class="error-message">Секція з такою назвою вже існує.</div>
                <?php endif; ?>
            </div>
